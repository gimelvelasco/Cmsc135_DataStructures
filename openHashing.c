...
void dic_search(char inkey[smax], struct node **h){
	struct node *p = *h;
	while(p != NULL){
...
void dic_modify(char inkey[smax], struct node **h){
	struct node *p = *h;
	while(p != NULL){
		if(strcmp(inkey, p->key) == 0){
...
void dic_delete(char inkey[smax], struct node **h){
	if(strcmp(inkey, (*h)->key) == 0){
		struct node *del = *h;
		*h = (*h)->next;
		free(del);
		...
		*/
		...
		switch(chc){
			...
			case 2:
				...
				gets(inkey);
				k = dic_convert(inkey);
				dic_modify(inkey,head[k]);
			break;
			case 3:
				printf("Input Key: ");
				gets(inkey);
				k = dic_convert(inkey);
				dic_search(inkey,head[k]);
			break;
			case 4:
				printf("Input Key: ");
				gets(inkey);
				k = dic_convert(inkey);
				dic_delete(inkey,head[k]);
			break;
			case 5:
